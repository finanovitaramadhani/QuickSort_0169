...
//swaps the element at index x with the element at indexs y
...
void q_short(int low, int high)
{
    ...
    while (i <= j) //step 10
    {
        ...
        while ((arr[i] <= pivot) && (j <= high)) //step 5
        {
            i++; //step 6
            cmp_count++;
        }
    ...
    //recursive call to start the left sub array
    q_short(low, j - 1); //step 12

    //recursive call to start the right sub array
